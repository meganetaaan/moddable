/*---
description: debug native runtime complex with single viewState computed.
flags: [async, module]
---*/
...
const viewState = createComputed(() => {
	...
	return {
		...
		hintText: (value === 0) ? "Tap the button to start" : (((value % 5) === 0) ? "Milestone reached" : "Keep tapping"),
		dotX: (value % 9) * 22,
	};
});
...
const mounted = mountPiuApplication(() => {
	...
	return node("application", { skin: appSkin },
		node("column", { left: 24, right: 24, top: 20, bottom: 20, skin: panelSkin },
			...
			node("container", { left: 24, right: 24, top: 12, height: 24, skin: trackSkin }, node("content", { left: state.dotX, top: 2, width: 24, height: 20, skin: dotSkin })),
		),
	);
});
