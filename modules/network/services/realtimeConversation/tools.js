...
export default class ToolRunner {
	...
	constructor(tools, {platform, send, onError, onResult, timeout = 30000}) {
		...
		this.definitions = tools.map(tool => {
			if (!tool || typeof tool.name !== "string" || !/^[a-zA-Z0-9_-]{1,64}$/.test(tool.name) ||
				this.#tools.has(tool.name) || typeof tool.execute !== "function" ||
				!tool.parameters || tool.parameters.type !== "object") throw new TypeError("Invalid tool definition");
			this.#tools.set(tool.name, tool.execute);
			return JSON.parse(JSON.stringify({type: "function", name: tool.name,
				description: tool.description ?? "", parameters: tool.parameters, strict: tool.strict ?? true}));
		});
		this.#platform = platform; this.#send = send; this.#error = onError;
		this.#notify = onResult; this.#timeout = timeout;
	}
	...
	async #execute(response, call) {
		const context = {callId: call.callId, responseId: response.id, delegationId: response.delegation,
			isCancelled: () => this.#closed || response.cancelled || timedOut};
		let timer, finish, timedOut = false;
		const stop = new Promise(resolve => { finish = resolve; });
		response.waits.add(finish);
		timer = this.#platform.setTimeout(() => { timedOut = true; finish({error: "tool_timeout"}); }, this.#timeout);
		const operation = Promise.resolve().then(async () => {
			...
			catch { return {error: "invalid_arguments"}; }
			try {
			...
			catch { return {error: "tool_execution_failed"}; }
		});
		try { return await Promise.race([operation, stop]); }
		finally { this.#platform.clearTimeout(timer); response.waits.delete(finish); }
	}
	...
	async #run(response) {
		try {
			...
			for (let i = 0; i < calls.length; i++) {
				if (this.#closed || response.cancelled) return;
				const call = calls[i], result = results[i];
				this.#send({type: "response.item.create", event_id: `tool_result_${++this.#serial}`,
					item: {type: "function_call_output", call_id: call.callId,
						output: result.output ?? JSON.stringify({error: result.error})}});
				this.#notify?.({name: call.name, callId: call.callId, responseId: response.id,
					delegationId: response.delegation, ...result});
			}
			if (calls.length && !this.#closed) this.#send({type: "response.create", event_id: `tool_continue_${++this.#serial}`});
		}
		catch (error) { if (!this.#closed) this.#error(error); }
		finally { this.#retire(response); }
	}
	...
	close() {
		...
		this.#tools.clear(); this.#seen.clear();
	}
}
