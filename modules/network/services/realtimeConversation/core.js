...
const DEFAULT_DELEGATION = {
	...
	responses: {
		model: "gpt-5.6-terra",
		instructions: "日本語の音声会話を支援してください。現在の情報が必要ならWeb検索し、根拠のある簡潔な結果を返してください。",
...
export class ConversationError extends Error {
	...
	get name() { return "ConversationError"; }
}
...
export default class Conversation {
	...
	constructor(options, platform) {
		...
		this.#options = {
			...
			instructions: options.instructions ?? "日本語で自然に簡潔に会話してください。調べ物や推論が必要な依頼はバックエンドに委譲してください。相手の発話を聞き、割り込みや言い直しに対応してください。",
	...
	get usage() { return this.#usage && {...this.#usage}; }
	get stats() { return this.#transport?.stats ?? this.#stats; }

	connect() {
		if (this.#state === "connecting" || this.#state === "connected") return this.#connection.promise;
		if (this.#state !== "idle") return Promise.reject(new ConversationError("Create a new conversation to reconnect", "closed", "connect"));
		...
		try {
			...
			this.#connectTimer = this.#after(60000, () => this.#fail(new ConversationError("Session startup timed out", "connect_timeout", "connect")));
	...
	setMuted(value) {
		...
		if (this.#state !== "connected") return Promise.reject(new ConversationError("Session is not connected", "not_connected", "mute"));
		...
		const pending = this.#mute = {...deferred(), value, id: `mute_${++this.#eventID}`};
		try {
			// Keep local capture gated until the remote command is acknowledged.
			this.#transport.setMuted(true);
			this.#muted = true;
			pending.timer = this.#after(5000, () => this.#rejectMute(new ConversationError("Mute acknowledgment timed out", "command_timeout", "mute")));
			this.#send({type: value ? "session.input_audio.mute" : "session.input_audio.unmute", event_id: pending.id});
		}
		catch (error) { this.#rejectMute(error); }
	...
	async #signal(sdp) {
		...
		try {
			...
			catch { throw new ConversationError("Invalid session response JSON", "invalid_json", "signaling"); }
		...
		finally { this.#signalPending = false; }
	}

	#receive(event) {
		if (this.#finishing || this.#result) return;
		try {
			if (event.type === "offer") { void this.#signal(event.sdp); return; }
			if (event.type === "error") throw new ConversationError(`WebRTC failure (${event.code})`, "transport_error", event.stage ?? "transport");
			if (event.type === "disconnected") throw new ConversationError("WebRTC disconnected before session.closed", "connection_lost", "transport");
			...
			switch (message.type) {
				...
				case "session.output_transcript.delta":
					if (typeof message.delta !== "string" || !Number.isFinite(message.start_ms) || !Number.isFinite(message.end_ms) || message.end_ms < message.start_ms)
						throw new ConversationError("Invalid transcript fragment", "invalid_transcript", "events");
					this.#notify("onTranscript", {role: message.type === "session.input_transcript.delta" ? "user" : "assistant", delta: message.delta, start_ms: message.start_ms, end_ms: message.end_ms});
					break;
				...
				case "session.input_audio.unmuted": {
					...
					if (pending && message.client_event_id === pending.id && (message.type === "session.input_audio.muted") === pending.value) {
				...
				case "error": {
					const error = new ConversationError(message.error?.message ?? "Live command rejected", message.error?.code ?? "server_error", "events");
					if (message.error?.client_event_id && this.#mute?.id === message.error.client_event_id) this.#rejectMute(error);
	...
	#send(message) { this.#transport.send(JSON.stringify(message)); }
	#sendClose() {
	...
	#fail(error) {
		if (this.#finishing || this.#result) return;
		if (!(error instanceof ConversationError)) error = new ConversationError(error?.message ?? "Conversation failed", "operation_failed", "transport");
	...
	async #hangup(id, key) {
		...
		catch { this.#notify("onError", new ConversationError("Session hangup was not confirmed", "hangup_failed", "close")); }
	}
	...
	#setState(state) { this.#state = state; this.#notify("onStateChanged", state); }
	#notify(name, value) {
		try { this.#options[name]?.(value); }
		catch (error) {
			if (name !== "onError") try { this.#options.onError?.(new ConversationError(error.message, "callback_error", "callback")); } catch {}
		}
	}
	#after(delay, callback) {
		const timer = this.#platform.setTimeout(() => { this.#timers.delete(timer); callback(); }, delay);
